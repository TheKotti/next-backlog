...
const CoverImage = ({
    ...
    const coverImage = cld
        .image(`covers/${game.coverImageId}`)
        .resize(fill().width(150).height(200))

    const formattedTitle = game.title.startsWith('The ')
        ? game.title.substring(4) + ', The'
        : game.title
    ...
    return (
        <div className={styles.imageContainer}>
            ...
            <div
                className={`d-flex flex-column justify-content-center align-items-center text-center gap-1 ${styles.overlay}`}
            >
                <a
                    href={game.igdbUrl}
                    target="_blank"
                ...
                {showHltb && <span>{getHltbString(game)}</span>}
                ...
                {isAdmin && <a href={`/recap?id=${game._id}`}>Recap</a>}
            </div>
        </div>
    )
}
