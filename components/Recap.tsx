...
export const Recap = ({ fetchedGame }: Props) => {
    ...
    const [timeToBeat, setTimeToBeat] = useState(
        formatTimeToBeat(fetchedGame.timeSpent, fetchedGame.additionalTimeSpent)
    )
    ...
    const coverImage = useMemo(
        () => cld.image(`covers/${game.coverImageId}`),
        [game.coverImageId]
    )
    ...
    const handleTimeChange = (value: string) => {
        ...
        const additionalTime = additionalTimeString
            ? parseFloat(additionalTimeString)
            : null
