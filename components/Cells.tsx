...
export const VodCell = ({ value, row }) => {
    ...
    return (
        <span style={{ color: 'darkcyan' }}>
            {value ? 'No vods available' : 'Not streamed'}
        </span>
    )
}
...
export const TitleCell = ({ value, row, showCovers }) => {
    ...
    const formattedTitle = value.startsWith('The ')
        ? value.substring(4) + ', The'
        : value
...
export const FinishedCell = ({ value, row }) => {
    const timeString =
        `${value}h ${row.original.additionalTimeSpent ? `+ ${row.original.additionalTimeSpent}h` : ''}`.trim()
    if (row.original.finished === 'Nope')
        return <>Did not finish {`(${timeString})`}</>
    if (row.original.finished === 'Yes')
        return <>Finished {`(${timeString})`}</>
